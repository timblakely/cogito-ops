...
| **llmkube 0.9.25** | Operator for inference servers: `Model`, `InferenceService`; runtimes `llamacpp`, `vllm`, `generic`. | syv-ai image as `runtime: generic`, Muse as `runtime: llamacpp`. `rolloutPolicy.waitForIdle`, `parallelSlots`, `speculativeDecoding`, `reasoningBudget`. GPUs UUID-bound via `NVIDIA_VISIBLE_DEVICES`. |
